...
public class NetworkCharacterControllerPrototypeCustom : NetworkTransform
{
    ...
    public float rotationAcceleration = 50.0f; // <<< 追加
    public float rotationBraking = 50.0f;

    public float backSpeedRate = 0.65f;
    ...
    [Networked, HideInInspector] // <<< 属性を追加
    public float AngularVelocityY { get; set; } // <<< 追加
    ...
    /// </summary>
    public virtual void Move(Vector3 direction, float inputY)
    {
        ...
        else
        {
            ...
            if (inputY < 0) // もし後退入力なら
            {
                currentMaxSpeed *= backSpeedRate; // 速度に倍率をかける
            }

            horizontalVel = Vector3.ClampMagnitude(horizontalVel + direction * acceleration * deltaTime, currentMaxSpeed);
    ...
    {
        var targetAngularVelocity = rotationY * rotationSpeed;

        AngularVelocityY = Mathf.Lerp(
            AngularVelocityY,
            targetAngularVelocity,
            Runner.DeltaTime * (rotationY != 0 ? rotationAcceleration : rotationBraking)
        );
        ...
        transform.Rotate(0, AngularVelocityY * Runner.DeltaTime, 0);
    }
}